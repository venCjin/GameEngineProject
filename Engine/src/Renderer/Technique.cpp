...
namespace sixengine {
	...
	void TransformBones(RendererCommand* command, std::vector<glm::mat4>* transform)
	{
		...
		// example
		//command->gameObject->GetComponent<AnimationState>()->GetName();
		command->gameObject->GetComponent<Mesh>()->GetModel()->BoneTransform(time, *transform);
	}
	...
	void UI::RenderText(Text* textR, glm::vec2 position)
	{
		...
		for (int i = 0; i < text.size(); i++)
		{
			...
			vertices.push_back({ { x + w, -y }, { ch.Offset + ch.Size.x / atlasSize.x, 0.0f } }); //prawy górny
			vertices.push_back({ { x, -y }, { ch.Offset, 0.0f } }); //lewy górny
			vertices.push_back({ { x, -y - h}, { ch.Offset, ch.Size.y / atlasSize.y } }); //lewy dolny

			vertices.push_back({ { x + w, -y }, { ch.Offset + ch.Size.x / atlasSize.x, 0 } }); //prawy górny
			vertices.push_back({ { x, -y - h }, { ch.Offset, ch.Size.y / atlasSize.y } }); //lewy dolny
